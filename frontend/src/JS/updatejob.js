...
// التحقق من الحقول عند الضغط على زر النشر
document.querySelector('#submit-button')?.addEventListener('click', function(event) {
    const titlee = document.querySelector('#job-titlee')?.value.trim();
    const namee = document.querySelector('#namee')?.value.trim();
    const jobTypee = document.querySelector('#job-typee')?.value.trim();
    const datee = document.querySelector('#job-datee')?.value.trim();
    const cityy = document.querySelector('#cityy')?.value.trim();
    const linkk = document.querySelector('#apply-linkk')?.value.trim();
    const descriptionn = document.querySelector('#descriptionn')?.value.trim();
    
    // التحقق من الحقول الفارغة
        if (!titlee || !namee || !jobTypee || !datee || !cityy || !linkk || !descriptionn) {
            alert('يرجى تعبئة جميع الحقول قبل النشر.');
            event.preventDefault();
            return;
        }
